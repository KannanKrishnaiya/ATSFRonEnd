...
export default function ChequeClearance() {
  ...
  const [ChequeClearanceTrans, SetChequeClearanceTrans] = useState([]);
  const [ChequeClearanceTransInput, setChequeClearanceTransInput] = useState({
  ...
  async function fetchData() {
    ...
    GetChequeClearanceRptAPI(Userdetails)
      .then((response) => {
        console.log("GetCurrentCDMLevelAPI", response);

        SetChequeClearanceTrans(response?.data.allMachinesCondition);
        setIsLoading(false);
      })
      .catch((err) => {
        setIsLoading(false);
  ...
  const Transactioncolumns = [
    ...
    {
      label: "Current Cheques Count",
      name: "depositedCheques",
      selector: (row) => row.depositedCheques,
      sortable: true,
      sort: true,
      sortDirection: true,
    },
